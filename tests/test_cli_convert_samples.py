...
def test_win2xcur_cli_converts_real_sample_without_installing(monkeypatch, windows_cursor_dir: Path, tmp_path: Path):
    monkeypatch.setattr(cli_convert, "check_image_magick_is_installed", lambda: True)
...
def test_convert_cli_raises_when_imagemagick_is_missing(monkeypatch, windows_cursor_dir: Path, tmp_path: Path):
    monkeypatch.setattr(cli_convert, "check_image_magick_is_installed", lambda: False)

    with pytest.raises(RuntimeError, match="ImageMagick"):
        cli_convert.win2xcur(str(windows_cursor_dir), output_path=tmp_path)
