...
_✨一站式鼠标指针转换与管理工具（图形界面版）_
  <p align="center">
    ...
    <a href="https://pypi.org/project/ani2xcur-gui" style="margin: 2px;">
      <img src="https://img.shields.io/pypi/v/ani2xcur-gui" alt="PyPI">
    </a>
    <a href="https://pypi.org/project/ani2xcur-gui" style="margin: 2px;">
      <img src="https://img.shields.io/pypi/pyversions/ani2xcur-gui.svg" alt="Python Version">
    </a>
  </p>
...
```bash
ani2xcur-gui --cli convert win2x <Windows 指针路径或者是鼠标指针压缩包下载链接>
```
...
```bash
ani2xcur-gui --cli convert x2win <Linux 指针路径或者是鼠标指针压缩包下载链接>
```
...
```bash
ani2xcur-gui --cli cursor install <指针路径>
```
...
```bash
ani2xcur-gui --cli cursor uninstall <指针名称>
```
...
```bash
ani2xcur-gui --cli cursor set theme <指针名称>
```

设置指针大小。

```bash
ani2xcur-gui --cli cursor set size <大小值>
```
...
```bash
ani2xcur-gui --cli cursor list
```

显示当前正在使用的指针主题和大小。

```bash
ani2xcur-gui --cli cursor status
```
...
```bash
ani2xcur-gui --cli cursor export <指针名称> <导出路径>
```
...
#### 自动下载并安装 ImageMagick
```bash
ani2xcur-gui --cli imagemagick install
```
...
#### 从系统中卸载 ImageMagick
```bash
ani2xcur-gui --cli imagemagick uninstall
```
