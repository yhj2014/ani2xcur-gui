...
- **依赖管理**: 内置 ImageMagick 安装和卸载功能，可用于兼容旧版转换环境或手动排障。
...
### Windows 转 Linux
将 Windows 指针主题转换为 Linux Xcursor 主题，兼容 X11 和 Wayland/XWayland 的标准 Xcursor 加载路径。
...
## ImageMagick 管理
Ani2xcur CLI 保留 ImageMagick 管理命令，用于兼容旧版转换环境或手动排障；当前内置转换器不再依赖 ImageMagick。
